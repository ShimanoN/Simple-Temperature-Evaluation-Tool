...
> **注意**: 本ツールはシングルチャネル設計です。
> 使用する MAX31855 の CS は GPIO5、microSD カードの CS は
> GPIO4 に固定しなければならず、異なるピン配置では SPI バス衝突が発生し、
> 温度値が異常になる問題が未回避です。
>
> 2026年2月27日のリファクタリングで、ハードウェアと、ソフトウェア双方で
> これらのピンを明示的に固定する実装が完了されました。
...
### MAX31855 モジュールのピン名について（注意）

- 一部モジュールではピン表記が `Vin`, `3VO`, `GND`, `DO`, `CS`, `CLK` のようになっています。実際の接続対応は以下の通りです。
  - `Vin` → モジュールの電源入力（モジュールにレギュレータがある場合は 5V を受けられることがある）
  - `3VO` → モジュールが出力する 3.3V（レギュレータ付きのモジュールでのみ存在）※通常は外部電源とは接続しない
  - `GND` → GND
  - `DO`  → MISO（データ出力）→ 本ドキュメントでは `SO` / `MISO` と記載している箇所に対応（例: GPIO19）
  - `CS`  → CS（チップセレクト）→ GPIO5
  - `CLK` → SCK（クロック）→ GPIO18

**電源に関する重要な注意**

- 安全のため、可能であれば `Vin` ではなく M5Stack の **3.3V ピン** を使って直接給電してください。多くの MAX31855 モジュールは 3.3V で動作します。誤って 5V を供給すると破損する可能性があります。
- `3VO` はモジュールの出力側の 3.3V であり、外部の 3.3V と短絡させるべきではありません。モジュールの回路図や仕様を確認してください。
- GND は常に共通にしてください（M5Stack と MAX31855 の GND を必ず接続）。

ピン表記が異なるモジュールを購入する際は、製品ページの回路図／説明を事前に確認してください。
...
### ステップ4: microSD カードの挿入

1. microSD カードが **FAT32** フォーマットであることを確認
2. M5Stack 本体**侧面（左下）** の microSD スロットにカードを差し込む
   - 端子表面（金色部分）を下向きにして挿入
3. **カチッと音がするまで**差し込む

> **ℹ️ SD カード CS ピンについて**: SD カードの CS ピン (GPIO4 / `TFCARD_CS_PIN`) は M5Stack 内部にハードウェア配線済みです。追加配線は不要です。コード内で `TFCARD_CS_PIN`（=GPIO4）を指定することで MAX31855（GPIO5）との SPI 衝突を回避しています。
...
3. **BtnA（左ボタン）を押す** → STATE: RUN に遷移（SD カードへの CSV 記録が自動開始）
4. 測定したい時間待つ（**推奨: 30秒以上**）
5. **BtnA を押す** → STATE: RESULT に遷移、ページ 1: 平均温度・サンプル数が表示される
6. **BtnB（中央ボタン）を押す** → ページ 2: 標準偏差・Range・Max/Min が表示される
7. 表示された**測定結果を記録**
   - CSV ファイルは SD カードに自動保存済み（`DATA_xxxx.csv`）
8. **BtnA を押す** → STATE: IDLE に戺る
9. 次の測定箇所へ移動
...
| **使用毎**   | SD カードの CSV データを PC へ定期的にバックアップする|
| **週１回**   | 配線の断線・接触不良を目視確認              |
| **月１回**   | 熱電対の接点の酸化・劣化を確認              |
| **月１回**   | 氷水0℃・沸騰水100℃で校正確認          |
| **月１回**   | SD カードの残り容量確認（必要に応じてフォーマット）         |
...
| **SD カードが検出されない**    | 未挿入またはフォーマット不匹合 | SD カードを差し込み、FAT32 フォーマットで再フォーマット |
| **CSV が保存されない**        | SD エラーまたは容量不足   | SD 画面のインジケーター（赤色）を確認、カード容量・フォーマットを確認 |
